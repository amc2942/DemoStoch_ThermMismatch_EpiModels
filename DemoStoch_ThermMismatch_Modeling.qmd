...
These code blocks will track the proportion of death at each temperature along the sequence, as well as the coefficient of variation in days in which the host clears the pathogen (return NA if no clearance events). also trying Shannon entropy to quantify variability. The entropy is defined as the probability-weighted average of the logarithmized probabilities for all possible values:
...
for (temp_idx in seq_along(temperatures)) {
  ...
  death_proportion <- round((death_count / num_sims), 3) #proportion of death events
  clearance_proportion <- round((clearance_count/num_sims), 3) # proportion of clearance events
  persist_proportion <- round((1 - death_proportion - clearance_proportion), 3)
  
  entropy <- 0  # Initialize
  
  # Calculate entropy 
  if (persist_proportion == 0) {
      # Handle the case where persist_proportion is 0
      if (death_proportion == 0 || clearance_proportion == 0) {
        entropy <- 0 # Entropy is 0 if all outcomes are the same (all death or all clearance)
      } else {
        entropy <- - (death_proportion * log(death_proportion) + (clearance_proportion * log(clearance_proportion)))
      }
  } else {
      # General case (persist_proportion is not 0)
      entropy <- - (death_proportion * log(death_proportion) + 
                    clearance_proportion * log(clearance_proportion) +
                    persist_proportion * log(persist_proportion))
  }
...
for (temp_idx in seq_along(temperatures)) {
  ...
  # Calculate Entropy (multinomial distribution and using natural logs)
  clearance_count <- sum(!is.na(clearance_days)) # Count clearance events
  death_proportion <- round((death_count / num_sims), 10) #proportion of death events
  clearance_proportion <- round((clearance_count/num_sims), 10) # proportion of clearance events
  persist_proportion <- round((1 - death_proportion - clearance_proportion), 10)
...
ent_plot <- ggplot(ent_data, aes(x = Temperature, y = Entropy, color = Legend_Label)) +
  geom_line(linewidth = 1) + 
  facet_wrap(~ Scenario, ncol = 1) + 
  scale_color_manual(values = c("Entropy" = "blue")) + 
    theme_bw() + 
  labs(x = "Temperature (°C)", y = "Entropy",
...
# Create the plot
entropy_propdeath_plot <- ggplot(combined_data, aes(x = Temperature, y = Value, linetype = Metric)) + 
  geom_line(size = 1, aes(color = Metric)) + 
  facet_wrap(~ Scenario, ncol = 1) +
  labs(x = "Temperature (°C)", y = "Entropy", linetype = NULL, color = NULL) +
  labs(title = "Entropy vs. Proportion of Deaths across a Thermal Gradient") +
  scale_y_continuous(sec.axis = sec_axis(~ ., name = "Proportion of Deaths")) +
...
entropy_propdeath_plot


#ggsave("Figures/Entropy_PropDeath.pdf", plot = entropy_propdeath_plot,
...
First, for the Coefficient of variation for the clearance days: 
...
Now for Coefficient of Variation for Proportion of deaths:
```{r, fig.height=8, fig.width=6, warning=FALSE}
# Data frame for CV results
prop_death_cv_data <- data.frame(
  Temperature = rep(temperatures, times = 5),
  Prop_Death_CoefVar = c(pd_cd_results_high$Prop_Death_CoefVar,
                     pd_cd_results_mid1$Prop_Death_CoefVar,
                     pd_cd_results_mid2$Prop_Death_CoefVar,
                     pd_cd_results_mid3$Prop_Death_CoefVar,
                     pd_cd_results_null$Prop_Death_CoefVar),
  Scenario = rep(c("High Mismatch", "Mid 1 Mismatch", "Mid 2 Mismatch", "Mid 3 Mismatch", "No Mismatch"), each = length(temperatures))
)

prop_death_cv_data$Legend_Label <- "Coefficient of Variation"

# Create the facet-wrapped plot for CV
prop_death_cv_plot <- ggplot(prop_death_cv_data, aes(x = Temperature, y = Prop_Death_CoefVar, color = Legend_Label)) +
  geom_line(linewidth = 1) +
  facet_wrap(~ Scenario, ncol = 1) +
  scale_color_manual(values = c("Coefficient of Variation" = "red")) + 
  theme_bw() +
  labs(x = "Temperature (°C)", y = "Coefficient of Variation",
       color = NULL, title = "Variability in Proportion of Deaths") + 
  theme(strip.text = element_text(size = 15),
        legend.position = "bottom",
        plot.title = element_text(hjust = 0.5, size = 18, face = "bold"),
        axis.title = element_text(size = 15),
...
#### Plot Variability in Prop Death next to Prop Death: 
```{r, fig.height=10, fig.width=10, warning=FALSE}
death_data_combine <- grid.arrange(death_plot, prop_death_cv_plot, ncol = 2)
invisible(death_data_combine)
```
