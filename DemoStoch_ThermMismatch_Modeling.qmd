...
pd_cd_results_high <- data.frame(Temperature = temperatures, Proportion_Deaths = NA, Entropy = NA)
...
for (temp_idx in seq_along(temperatures)) {
  ...
  # Check for death *FIRST*
    if (max(sim_results$V_vec, na.rm = TRUE) >= death_threshold) {
      ...
      clearance_days[sim] <- NA  # Mark as NA if death occurs *before* clearance
    } else { #if death does not occur, then proceed to check for clearance
      # Check for pathogen clearance (exact comparison)
...
pd_cd_results_mid1 <- data.frame(Temperature = temperatures, Proportion_Deaths = NA, Entropy = NA)
...
pd_cd_results_mid2 <- data.frame(Temperature = temperatures, Proportion_Deaths = NA, Entropy = NA)
...
pd_cd_results_mid3 <- data.frame(Temperature = temperatures, Proportion_Deaths = NA, Entropy = NA)
...
```{r, warning=FALSE}
# Data frame to store both metrics for mid1 mismatch
pd_cd_results_null <- data.frame(Temperature = temperatures, Proportion_Deaths = NA, Entropy = NA)
...
cv_plot <- ggplot(ent_data, aes(x = Temperature, y = Entropy, color = Legend_Label)) +
  geom_line(linewidth = 1) + 
  facet_wrap(~ Scenario, ncol = 1) + 
  scale_color_manual(values = c("Variability in Outcomes" = "blue")) + 
    theme_bw() + 
  labs(x = "Temperature (°C)", y = "Shannon Entropy",
       color = NULL, title = "C. Variability in Outcomes") + 
  theme(strip.text = element_text(size = 15),
        legend.position = "bottom",
        plot.title = element_text(hjust = 0.5, size = 20, face = "bold"),
        axis.title = element_text(size = 17),
