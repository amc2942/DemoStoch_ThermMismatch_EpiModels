...
cv_data <- data.frame(
  ...
  Scenario = rep(c("High Mismatch", "Mid 1 Mismatch", "Mid 2 Mismatch", "Mid 3 Mismatch", "No Mismatch"), each = length(temperatures_smaller))
)
...
phi_vals_hm <- seq(2.5, 3.15, length.out = 25)
alpha_vals_hm <- seq(2.5, 3.15, length.out = 25)
...
phi_alpha_range_plot <- ggplot(heatmap_data, aes(x = phi, y = alpha, fill = Entropy)) +
  ...
  # Add points and lines with custom colors (and NO inherited aesthetics)
  geom_point(data = all_param_values, aes(x = Phi, y = Alpha, color = Scenario, 
                                          shape = Scenario), size = 5, inherit.aes = FALSE) +
  geom_path(data = all_param_values, aes(x = Phi, y = Alpha, color = Scenario, group = Scenario), 
            linewidth = 1, alpha = 1, inherit.aes = FALSE)+
  scale_color_manual(values = c(
    "High Mismatch" = "black",  
    "Mid 1 Mismatch" = "navyblue",  
    "Mid 2 Mismatch" = "red", 
    "Mid 3 Mismatch" = "magenta", 
    "No Mismatch" = "orange"   
  )) +
...
# Plot for Heatmap of Entropy of Model Outcomes with different values of Phi and Psi

This chunk takes a while to run, 25 x 25 parameter combinations with 100 simulations each....
```{r, warning = FALSE}
# Define synthetic range for phi and alpha
phi_vals_hm <- seq(2.25, 2.65, length.out = 25)
psi_vals_hm <- seq(0.9, 1.15, length.out = 25)

# Dataframe for grid results
death_prop_grid2 <- expand.grid(phi = phi_vals_hm, psi = psi_vals_hm)
death_prop_grid2$Entropy <- NA
...
for (i in seq_len(nrow(death_prop_grid2))) {
  ...
  # Store entropy
  death_prop_grid2$Entropy[i] <- entropy
}
```
