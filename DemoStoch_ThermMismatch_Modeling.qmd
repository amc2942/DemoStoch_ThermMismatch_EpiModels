...
for (temp_idx in seq_along(temperatures)) {
  ...
  if (proportion_deaths == 0) {
    ...
    min_deaths <- 4  # minimum threshold
  
    if (death_count >= min_deaths) {  # Check for minimum threshold
        sd_prop_deaths <- sd(prop_deaths_vec)
        mean_prop_deaths <- mean(prop_deaths_vec)
        prop_death_cv <- sd_prop_deaths / mean_prop_deaths
    } else {
        prop_death_cv <- NA # Not enough deaths for reliable calculation
    }
  }
