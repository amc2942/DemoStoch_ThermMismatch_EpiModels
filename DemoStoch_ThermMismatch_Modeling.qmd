...
scenario_by_temp_plot_individualsims <- ggplot(individual_simulations, aes(x = Day)) +
  ...
  labs(title = "A. Results by Mismatch Scenario and Temperature (100 Simulations)",
       x = "Days", 
       y = "Within-Host Cell Density/Load") + 
  theme_bw() + 
  theme(legend.position = "bottom", 
...
# Track the Proportion of deaths across thermal gradient and coefficient of variation in clearance days:

These code blocks will track the proportion of death at each temperature along the sequence, as well as the coefficient of variation in days in which the host clears the pathogen (return NA if no clearance events).

#### High Mismatch: 
```{r, warning=FALSE}
# Data frame to store both metrics for high mismatch
pd_cd_results_high <- data.frame(Temperature = temperatures, Proportion_Deaths = NA, Coef_Variation = NA)
...
for (temp_idx in seq_along(temperatures)) {
  ...
  for (sim in 1:num_sims) {
    ...
    # Check for pathogen clearance (first day V_vec reaches 0)
    clearance_day <- which(sim_results$V_vec == 0)[1]  # First day it happens
    clearance_days[sim] <- ifelse(is.na(clearance_day), NA, clearance_day)
  }
  
  # Compute proportion of deaths
  pd_cd_results_high$Proportion_Deaths[temp_idx] <- death_count / num_sims
  
  # Compute coefficient of variation for clearance
  mean_clearance <- mean(clearance_days, na.rm = TRUE)
  sd_clearance <- sd(clearance_days, na.rm = TRUE)
  
  pd_cd_results_high$Coef_Variation[temp_idx] <- ifelse(
    sum(!is.na(clearance_days)) == 0,  # If no clearance events occurred
    0,  # Return 0 instead of NA
    sd_clearance / mean_clearance
  )
}
```
...
pd_cd_results_mid1 <- data.frame(Temperature = temperatures, Proportion_Deaths = NA, Coef_Variation = NA)
...
for (temp_idx in seq_along(temperatures)) {
  ...
  # Compute proportion of deaths
  pd_cd_results_mid1$Proportion_Deaths[temp_idx] <- death_count / num_sims
  
  # Compute coefficient of variation for clearance
  mean_clearance <- mean(clearance_days, na.rm = TRUE)
  sd_clearance <- sd(clearance_days, na.rm = TRUE)
  
  pd_cd_results_mid1$Coef_Variation[temp_idx] <- ifelse(
    sum(!is.na(clearance_days)) == 0,  # If no clearance events occurred
    0,  # Return 0 
    sd_clearance / mean_clearance
  )
...
pd_cd_results_mid2 <- data.frame(Temperature = temperatures, Proportion_Deaths = NA, Coef_Variation = NA)
...
for (temp_idx in seq_along(temperatures)) {
  ...
  # Compute proportion of deaths
  pd_cd_results_mid2$Proportion_Deaths[temp_idx] <- death_count / num_sims
  
  # Compute coefficient of variation for clearance
  mean_clearance <- mean(clearance_days, na.rm = TRUE)
  sd_clearance <- sd(clearance_days, na.rm = TRUE)
  
  pd_cd_results_mid2$Coef_Variation[temp_idx] <- ifelse(
    sum(!is.na(clearance_days)) == 0,  # If no clearance events occurred
    0,  # Return 0 
    sd_clearance / mean_clearance
  )
...
pd_cd_results_mid3 <- data.frame(Temperature = temperatures, Proportion_Deaths = NA, Coef_Variation = NA)
...
for (temp_idx in seq_along(temperatures)) {
  ...
  # Compute proportion of deaths
  pd_cd_results_mid3$Proportion_Deaths[temp_idx] <- death_count / num_sims
  
  # Compute coefficient of variation for clearance
  mean_clearance <- mean(clearance_days, na.rm = TRUE)
  sd_clearance <- sd(clearance_days, na.rm = TRUE)
  
  pd_cd_results_mid3$Coef_Variation[temp_idx] <- ifelse(
    sum(!is.na(clearance_days)) == 0,  # If no clearance events occurred
    0,  # Return 0 
    sd_clearance / mean_clearance
  )
...
#### Null Mismatch: 
```{r, warning=FALSE}
# Data frame to store both metrics for no mismatch
pd_cd_results_null <- data.frame(Temperature = temperatures, Proportion_Deaths = NA, Coef_Variation = NA)
...
for (temp_idx in seq_along(temperatures)) {
  ...
  # Compute proportion of deaths
  pd_cd_results_null$Proportion_Deaths[temp_idx] <- death_count / num_sims
  
  # Compute coefficient of variation for clearance
  mean_clearance <- mean(clearance_days, na.rm = TRUE)
  sd_clearance <- sd(clearance_days, na.rm = TRUE)
  
  pd_cd_results_null$Coef_Variation[temp_idx] <- ifelse(
    sum(!is.na(clearance_days)) == 0,  # If no clearance events occurred
    0,  # Return 0 
    sd_clearance / mean_clearance
  )
...
death_plot <- ggplot(death_data, aes(x = Temperature, y = Proportion_Deaths, color = Legend_Label)) + 
  ...
  labs(x = "Temperature (°C)", y = "Proportion of Deaths",
       color = NULL, title = "B. Deaths Across Thermal Gradient") + 
...
# Plot the coefficient of variation for each thermal mismatch scenario
...
cv_plot <- ggplot(cv_data, aes(x = Temperature, y = Coef_Variation, color = Legend_Label)) +
  geom_line(linewidth = 1) + 
  facet_wrap(~ Scenario, ncol = 1) + 
  scale_color_manual(values = c("Coefficient of Variation" = "blue")) + 
    theme_bw() + 
  labs(x = "Temperature (°C)", y = "Coefficient of Variation",
       color = NULL, title = "C. Variability in Pathogen Clearance Across Thermal Gradient") + 
  theme(strip.text = element_text(size = 15),
        legend.position = "bottom",
        plot.title = element_text(hjust = 0.5, size = 20, face = "bold"),
        axis.title = element_text(size = 17),
