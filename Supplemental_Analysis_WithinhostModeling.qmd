---
title: "Supplemental_Analysis_WithinhostModeling"
...
# Quadratic Thermal Performance Curves
```{r}
...
Estimating the Range of Parameter Psi (Immune component growth rate in response to virus)
```{r}
...
# Calculate corresponding values of psi
...
# Peak value of the psi Parameter
...
psi_plot <- ggplot(plot_data, aes(x = Temperature, y = Psi)) +
  ...
  labs(x = "Temperature (°C)", y = "Psi (Host) Values") + 
  geom_vline(xintercept = temp_peak_psi, color = "blue", linetype = "dashed") +
  theme_bw() + 
  theme(axis.text = element_text(size = 15)) + 
  theme(axis.title = element_text(size = 15))
...
plot_data_long <- plot_data %>% pivot_longer(cols = c(Phi, Psi), names_to = "Parameter", values_to = "Value")
...
combined_plot <- ggplot(plot_data_long, aes(x = Temperature, y = Value, color = Parameter)) +
  ...
  geom_vline(xintercept = temp_peak_psi, color = "blue", linetype = "dashed") + # For Psi peak
  ...
  scale_color_manual(values = c("Phi" = "red", "Psi" = "blue"))

combined_plot
```

# Generate Thermal Mismatch Scenarios
...
# ------------------------------------------------
# NEXT: Psi value for High Mismatch
# Set temperature range
tmin_psi_high <- -5
tmax_psi_high <- 24

# Calculate corresponding values of psi
psi_values_high <- estimate_psi(temperatures, q_psi, tmin_psi_high, tmax_psi_high)

# Min-Max Normalization
norm_psi_high <- (psi_values_high - min(psi_values_high)) / (max(psi_values_high) - min(psi_values_high))

# Peak value of the psi Parameter
temp_peak_psi_high <- (tmin_psi_high + tmax_psi_high) / 2
peak_psi_high <- estimate_psi(temp_peak_psi_high, q_psi, tmin_psi_high, tmax_psi_high)
```
...
# ------------------------------------------------
# NEXT: Psi value for Mid 1 Mismatch
# Set temperature range
tmin_psi_mid1 <- -4.25
tmax_psi_mid1 <- 24.75

# Calculate corresponding values of psi
psi_values_mid1 <- estimate_psi(temperatures, q_psi, tmin_psi_mid1, tmax_psi_mid1)

# Min-Max Normalization
norm_psi_mid1 <- (psi_values_mid1 - min(psi_values_mid1)) / (max(psi_values_mid1) - min(psi_values_mid1))

# Peak value of the psi Parameter
temp_peak_psi_mid1 <- (tmin_psi_mid1 + tmax_psi_mid1) / 2
peak_psi_mid1 <- estimate_psi(temp_peak_psi_mid1, q_psi, tmin_psi_mid1, tmax_psi_mid1)
```
...
# ------------------------------------------------
# NEXT: psi value for Mid 2 Mismatch
# Set temperature range
tmin_psi_mid2 <- -3.5
tmax_psi_mid2 <- 25.5

# Calculate corresponding values of psi
psi_values_mid2 <- estimate_psi(temperatures, q_psi, tmin_psi_mid2, tmax_psi_mid2)

# Min-Max Normalization
norm_psi_mid2 <- (psi_values_mid2 - min(psi_values_mid2)) / (max(psi_values_mid2) - min(psi_values_mid2))

# Peak value of the psi Parameter
temp_peak_psi_mid2 <- (tmin_psi_mid2 + tmax_psi_mid2) / 2
peak_psi_mid2 <- estimate_psi(temp_peak_psi_mid2, q_psi, tmin_psi_mid2, tmax_psi_mid2)
```
...
# ------------------------------------------------
# NEXT: Psi value for Mid 3 Mismatch
# Set temperature range
tmin_psi_mid3 <- -2.75
tmax_psi_mid3 <- 26.25

# Calculate corresponding values of psi
psi_values_mid3 <- estimate_psi(temperatures, q_psi, tmin_psi_mid3, tmax_psi_mid3)

# Min-Max Normalization
norm_psi_mid3 <- (psi_values_mid3 - min(psi_values_mid3)) / (max(psi_values_mid3) - min(psi_values_mid3))

# Peak value of the psi Parameter
temp_peak_psi_mid3 <- (tmin_psi_mid3 + tmax_psi_mid3) / 2
peak_psi_mid3 <- estimate_psi(temp_peak_psi_mid3, q_psi, tmin_psi_mid3, tmax_psi_mid3)
```
...
# ------------------------------------------------
# NEXT: Psi value for No Mismatch
# Set temperature range
tmin_psi_null <- -2
tmax_psi_null <- 27

# Calculate corresponding values of psi
psi_values_null <- estimate_psi(temperatures, q_psi, tmin_psi_null, tmax_psi_null)

# Min-Max Normalization
norm_psi_null <- (psi_values_null - min(psi_values_null)) / (max(psi_values_null) - min(psi_values_null))

# Peak value of the psi Parameter
temp_peak_psi_null <- (tmin_psi_null + tmax_psi_null) / 2
peak_psi_null <- estimate_psi(temp_peak_psi_null, q_psi, tmin_psi_null, tmax_psi_null)
```
...
param_function_plot_data <- data.frame(
  ...
  Normalized_Psi = c(norm_psi_high, norm_psi_mid1, norm_psi_mid2, norm_psi_mid3, norm_psi_null),
...
optimal_temps <- data.frame(
  ...
  Temp_Peak_Psi_opt = c(temp_peak_psi_high, temp_peak_psi_mid1, temp_peak_psi_mid2, temp_peak_psi_mid3, temp_peak_psi_null)
)

# pivot for ggplot
param_function_plot_data_long <- param_function_plot_data %>% 
  pivot_longer(cols = c(Normalized_Phi, Normalized_Psi), 
...
quad_param_plot_data$Parameter <- factor(quad_param_plot_data$Parameter, 
                                          levels = c("Normalized_Phi", "Normalized_Psi"))

# Create the plot
quad_param_plot2 <- ggplot(quad_param_plot_data, 
  ...
  scale_color_manual(values = c("Normalized_Phi" = "red", "Normalized_Psi" = "blue"),
                     labels = c("Phi (Virus)", "Psi (Host)")) + 
...
for (temp in temp_checks) {
  ...
  psi_high <- estimate_psi(temp, q_psi, tmin_psi_high, tmax_psi_high)
...
for (temp in temp_checks) {
  ...
  psi_mid1 <- estimate_psi(temp, q_psi, tmin_psi_mid1, tmax_psi_mid1)
...
for (temp in temp_checks) {
  ...
  psi_mid2 <- estimate_psi(temp, q_psi, tmin_psi_mid2, tmax_psi_mid2)
...
for (temp in temp_checks) {
  ...
  psi_mid3 <- estimate_psi(temp, q_psi, tmin_psi_mid3, tmax_psi_mid3)
...
for (temp in temp_checks) {
  ...
  psi_null <- estimate_psi(temp, q_psi, tmin_psi_null, tmax_psi_null)
...
# Tracking outcomes across thermal gradient:
...
for (temp_idx in seq_along(temperatures_smaller)) {
  ...
  pd_cd_results_high$Psi[temp_idx] <- psi_high
...
for (temp_idx in seq_along(temperatures_smaller)) {
  ...
  pd_cd_results_mid1$Psi[temp_idx] <- psi_mid1
...
for (temp_idx in seq_along(temperatures_smaller)) {
  ...
  pd_cd_results_mid2$Psi[temp_idx] <- psi_mid2
...
for (temp_idx in seq_along(temperatures_smaller)) {
  ...
  pd_cd_results_mid3$Psi[temp_idx] <- psi_mid3
...
for (temp_idx in seq_along(temperatures_smaller)) {
  ...
  pd_cd_results_null$Psi[temp_idx] <- psi_null
...
death_plot <- ggplot(death_data, aes(x = Temperature, y = Proportion_Deaths, color = Legend_Label)) +
  ...
  geom_blank(aes(y = 0.5, color = "50% of Simulations Die")) + 
  scale_color_manual(values = c("Proportion of Deaths" = "red", "50% of Simulations Die" = "black")) +
  facet_wrap(~ Scenario, ncol = 1) +
...
ent_plot <- ggplot(ent_data, aes(x = Temperature, y = Entropy, color = Legend_Label)) +
  ...
  scale_color_manual(values = c("Entropy" = "blue", "50% of Simulations Die" = "black")) +
...
#ggsave("Figures/FullFigure_PhiPsi_supp.pdf", plot = all_plots_individual_sims, 
#       device = "pdf", width = 25, height = 15)
```

# Plot for Heatmap of the Entropy of Model Outcomes with different values of Phi and Psi
...
psi_vals_hm <- seq(0.8, 1.05, length.out = 25)

# Dataframe for grid results
death_prop_grid <- expand.grid(phi = phi_vals_hm, psi = psi_vals_hm)
...
heatmap_data <- death_prop_grid[, c("phi", "psi", "Entropy")]

phi_psi_range_plot <- ggplot(heatmap_data, aes(x = phi, y = psi, fill = Entropy)) +
  geom_tile() +
  scale_fill_viridis_c(option = "mako", direction = -1) +
  theme_bw() +
  labs(title = "Outcome Variability across a Range of Parameter Values",
       x = expression(paste("Viral Replication Rate (", phi, ") Range")),
       y = expression(paste("Immune Response Growth Rate ( ", psi, ") Range")),
       fill = "Entropy") +
  ...
  geom_path(data = all_param_gradient, aes(x = Phi, y = Psi, color = Scenario),
...
#ggsave("Figures/phi_psi_range_plot_supp.pdf", plot = phi_psi_range_plot,
